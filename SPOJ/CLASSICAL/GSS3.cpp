...
int findId(int id, int l, int r, int pos)
{
    if(l==r && l==pos)
        return  id;
    ...
    if(pos<=mid)
        return findId(LEFT(id),l,mid,pos);
    else
        return findId(RIGHT(id),mid+1,r,pos);
}

void update(int pos, int val)
{
    int id = findId(1,1,n,pos);
    int l,r;

    t[id].sum = t[id].prefix = t[id].suffix = t[id].max_sum = a[pos] = val;
